...
export const FloatingActions: React.FC<FloatingActionsProps> = ({ onOpenEmail }) => {
  ...
  return (
    <aside aria-label="Contacto flotante" className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-3 pointer-events-auto">
      
      {/* 1. Email Button (Same size as WhatsApp, White Logo) */}
      <button
        onClick={onOpenEmail}
        className="group relative flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-[#2563eb] shadow-xl hover:shadow-blue-500/30 hover:-translate-y-1 active:translate-y-0 transition-all duration-200"
        title="Enviar Correo Electrónico"
      >
        {/* Pure White Email Logo SVG */}
        <svg 
          className="w-7 h-7 sm:w-8 sm:h-8 fill-white transition-transform group-hover:scale-110 duration-200" 
          viewBox="0 0 24 24" 
        ...
        {/* Hover Tooltip Label */}
        <span className="absolute right-18 px-2.5 py-1 bg-slate-900 text-white text-xs font-semibold rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none shadow-md">
          Correo Electrónico
        </span>
      </button>

      {/* 2. Official WhatsApp Button (Identical Size) */}
      <a
        href={whatsappUrl}
        target="_blank"
        rel="noreferrer"
        className="group relative flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-[#25D366] shadow-xl hover:shadow-emerald-500/30 hover:-translate-y-1 active:translate-y-0 transition-all duration-200"
